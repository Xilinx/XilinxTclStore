...
proc usf_create_do_file { simulator do_filename } {
  ...
  set saif {}
  set uut {}
  [catch {set uut [get_property -quiet "vcs.simulate.uut" $a_sim_vars(fs_obj)]} msg]
  set saif_scope [get_property "vcs.simulate.saif_scope" $a_sim_vars(fs_obj)]
  if { {} != $saif_scope } {
    set uut $saif_scope
  }
  set saif [get_property "vcs.simulate.saif" $a_sim_vars(fs_obj)]
  if { {} != $saif } {
    if { {} == $uut } {
  ...
  # write tcl post hook
  set tcl_post_hook [get_property "vcs.simulate.tcl.post" $a_sim_vars(fs_obj)]
  if { {} != $tcl_post_hook } {
  ...
  set rt [string trim [get_property "vcs.simulate.runtime" $a_sim_vars(fs_obj)]]
  if { $a_sim_vars(b_int_en_vitis_hw_emu_mode) } {
...
proc usf_get_files_for_compilation_post_sim { global_files_str_arg } {
  ...
  if { {} != $a_sim_vars(s_netlist_file) } {
    ...
    set cmd_str [usf_get_file_cmd_str $a_sim_vars(s_netlist_file) $file_type false {} l_incl_dirs_opts]
    if { {} != $cmd_str } {
      lappend files $cmd_str
...
proc usf_launch_script { simulator step } {
  ...
  set extn [xcs_get_script_extn "vcs"]
  ...
  if { $a_sim_vars(b_scripts_only) } {
    send_msg_id USF-VCS-061 INFO "Script generated:[file normalize [file join $a_sim_vars(s_launch_dir) $scr_file]]"
    return 0
  }
  ...
  send_msg_id USF-VCS-062 INFO "Executing '${display_step}' step in '$a_sim_vars(s_launch_dir)'"
  set results_log {}
  switch $step {
    {compile} -
    {elaborate} {
  ...
  switch $tool {
    "vlogan" {
      ...
      if { [llength $verilog_defines] > 0 } {
        xcs_append_define_generics "vcs" $verilog_defines $tool opts
      }
...
proc usf_get_file_cmd_str { file file_type b_xpm global_files_str l_incl_dirs_opts_arg {xpm_library {}} {xv_lib {}}} {
  ...
  set compiler [xcs_get_compiler_name "vcs" $file_type]
  set arg_list [list]
  ...
  xcs_append_other_options $compiler $global_files_str $file_type arg_list
...
proc usf_get_source_from_repo { ip_file orig_src_file launch_dir b_is_static_arg b_is_dynamic_arg } {
  ...
  if { [lsearch -exact $used_in_values "ipstatic"] == -1 } {
    ...
    if { !$b_found_in_repo } {
      #send_msg_id USF-VCS-024 WARNING "Expected IP user file does not exist:'$repo_src_file'!, using from default location:'$full_src_file_path'"
    }
  } else {
