...
proc usf_launch_script { simulator step } {
  ...
  if { $a_sim_vars(b_scripts_only) } {
    send_msg_id Vivado-ModelSim-068 INFO "Scripts generated."
    return 0
  }
...
proc usf_generate_comp_file_for_simulation { comp_file runs_to_launch_arg } {
  ...
  send_msg_id Vivado-ModelSim-081 INFO "Processing IP '$ip_name' for simulation models/functional netlist...\n"
  ...
  if { [get_property "IS_IP_BEHAV_LANG_SUPPORTED" $comp_file] } {
    send_msg_id Vivado-ModelSim-082 INFO "IP supports '$ts' language simulation models\n"
    # does ip generated simulation products? if not, generate them
    if { ![get_property "IS_IP_GENERATED_SIM" $comp_file] } {
      send_msg_id Vivado-ModelSim-083 INFO "IP is not generated for simulation. Generating simulation products...\n"
      generate_target {simulation} [get_files $comp_file] -force
    } else {
      send_msg_id Vivado-ModelSim-084 INFO "IP is upto date for simulation\n"
    }
  } elseif { [get_property "GENERATE_SYNTH_CHECKPOINT" $comp_file] } {
    send_msg_id Vivado-ModelSim-085 INFO "IP 'GENERATE_SYNTH_CHECKPOINT' is set\n"
    # make sure ip is up-to-date
    if { ![get_property "IS_IP_GENERATED" $comp_file] } { 
      send_msg_id Vivado-ModelSim-086 INFO "Generating 'all' products...\n"
      generate_target {all} [get_files $comp_file] -force
      send_msg_id Vivado-ModelSim-087 INFO "Generating 'netlist' with IP block-fileset runs...\n"
      usf_generate_ip_netlist $comp_file runs_to_launch
    } else {
      send_msg_id Vivado-ModelSim-088 INFO "IP is upto date for all products\n"
    }
  } else {
    ...
    send_msg_id Vivado-ModelSim-089 WARNING "$error_msg\n"
    #return 1
  }
  send_msg_id Vivado-ModelSim-090 INFO "Processing complete:'$ip_name'\n"
}
