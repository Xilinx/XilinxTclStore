...
proc usf_questa_setup_simulation { args } {
  ...
  #::tclapp::xilinx::questa::usf_print_args

  # write functional/timing netlist for post-* simulation
  ::tclapp::xilinx::questa::usf_write_design_netlist
...
proc usf_questa_init_simulation_vars {} {
  ...
  set a_questa_sim_vars(b_32bit)            [get_property "QUESTA.COMPILE.32BIT" $fs_obj]
...
proc usf_questa_create_do_file_for_compilation { do_file } {
  ...
  if { [::tclapp::xilinx::questa::usf_compile_glbl_file "questa" $b_load_glbl $::tclapp::xilinx::questa::a_sim_vars(l_design_files)] } {
    ::tclapp::xilinx::questa::usf_copy_glbl_file
...
proc usf_questa_get_elaboration_cmdline {} {
  ...
  if { [get_param project.writeNativeScriptForUnifiedSimulation] } {
    if { ($::tclapp::xilinx::questa::a_questa_sim_vars(b_32bit)) || ({32} == $os_type) } {
      lappend arg_list {-32}
    } else {
      lappend arg_list {-64}
    }
  ...
  if { [::tclapp::xilinx::questa::usf_contains_verilog $design_files] } {    
...
proc usf_questa_create_do_file_for_simulation { do_file } {
  ...
  if { {} != $saif } {
    ...
    if { ({functional} == $::tclapp::xilinx::questa::a_sim_vars(s_type)) || \
      ...
      puts $fh "power add -r -in -inout -out -internal [::tclapp::xilinx::questa::usf_resolve_uut_name uut]\n"
    } else {
      puts $fh "power add -in -inout -out -internal [::tclapp::xilinx::questa::usf_resolve_uut_name uut]\n"
    }
  }
