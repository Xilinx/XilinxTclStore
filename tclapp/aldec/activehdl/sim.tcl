...
package require ::tclapp::aldec::activehdl::helpers
...
proc setup { args } {
  ...
  ::tclapp::aldec::activehdl::usf_init_vars
...
proc compile { args } {
  ...
  ::tclapp::aldec::activehdl::usf_launch_script "modelsim" $step
}
...
proc usf_modelsim_setup_simulation { args } {
  ...
  if { [catch {::tclapp::aldec::activehdl::usf_set_simulator_path "modelsim"} err_msg] } {
    send_msg_id Vivado-ModelSim-005 ERROR "$err_msg\n"
  }

  # set the simulation flow
  ::tclapp::aldec::activehdl::usf_set_simulation_flow

  # set default object
  if { [::tclapp::aldec::activehdl::usf_set_sim_tcl_obj] } {
    puts "failed to set tcl obj"
    return 1
  }

  # print launch_simulation arg values
  #::tclapp::aldec::activehdl::usf_print_args

  # write functional/timing netlist for post-* simulation
  ::tclapp::aldec::activehdl::usf_write_design_netlist

  # prepare IP's for simulation
  ::tclapp::aldec::activehdl::usf_prepare_ip_for_simulation

  # find/copy modelsim.ini file into run dir
  #usf_modelsim_verify_compiled_lib

  # fetch the compile order for the specified object
  ::tclapp::aldec::activehdl::usf_get_compile_order_for_obj
...
proc usf_modelsim_setup_args { args } {
  ...
  for {set i 0} {$i < [llength $args]} {incr i} {
    ...
    switch -regexp -- $option {
      "-simset"         { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_simset) [lindex $args $i] }
      "-mode"           { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_mode) [lindex $args $i] }
      "-type"           { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_type) [lindex $args $i] }
      "-scripts_only"   { set ::tclapp::aldec::activehdl::a_sim_vars(b_scripts_only) 1 }
      "-of_objects"     { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_comp_file) [lindex $args $i]}
      "-absolute_path"  { set ::tclapp::aldec::activehdl::a_sim_vars(b_absolute_path) 1 }
      "-install_path"   { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_install_path) [lindex $args $i] }
      "-batch"          { set ::tclapp::aldec::activehdl::a_sim_vars(b_batch) 1 }
      "-run_dir"        { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir) [lindex $args $i] }
      "-int_os_type"    { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_int_os_type) [lindex $args $i] }
      "-int_debug_mode" { incr i;set ::tclapp::aldec::activehdl::a_sim_vars(s_int_debug_mode) [lindex $args $i] }
      default {
  ...
  if { ![file isdirectory $::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir)] } {
    set ::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir) [get_property "DIRECTORY" [current_project]]
  }
}
...
proc usf_modelsim_verify_compiled_lib {} {
  ...
  if { {} == $compiled_lib_dir } {
    set file [file normalize [file join $::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir) $ini_file]]
    if { ! [file exists $file] } {
    ...
    if { [file exists $ini_file_path] } {
      if {[catch {file copy -force $ini_file_path $::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir)} error_msg] } {
        send_msg_id Vivado-modelsim-010 ERROR "failed to copy file ($ini_file): $error_msg\n"
      } else {
        send_msg_id Vivado-modelsim-011 INFO "File '$ini_file_path' copied to run dir:'$::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir)'\n"
      }
    }
  }
}
...
proc usf_modelsim_write_setup_files {} {
  ...
  set dir $::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir)
...
proc usf_modelsim_create_log_do_file { file } {
  ...
  puts $fh "log *"
  if { [::tclapp::aldec::activehdl::usf_contains_verilog] } {
...
proc usf_modelsim_create_do_file_for_compilation { do_file } {
  ...
  set fs_obj [get_filesets $::tclapp::aldec::activehdl::a_sim_vars(s_simset)]
  set b_absolute_path $::tclapp::aldec::activehdl::a_sim_vars(b_absolute_path)
...
#  puts $fh "vlib msim\n"
  ...
  set files [::tclapp::aldec::activehdl::usf_uniquify_cmd_str [::tclapp::aldec::activehdl::usf_get_files_for_compilation]]
  ...
  # compile glbl file
  set b_load_glbl [get_property "MODELSIM.COMPILE.LOAD_GLBL" [get_filesets $::tclapp::aldec::activehdl::a_sim_vars(s_simset)]]
  if { [::tclapp::aldec::activehdl::usf_compile_glbl_file "modelsim" $b_load_glbl] } {
    ::tclapp::aldec::activehdl::usf_copy_glbl_file
    set top_lib [::tclapp::aldec::activehdl::usf_get_top_library]
...
proc usf_modelsim_get_elaboration_cmdline { step } {
  ...
  set top $::tclapp::aldec::activehdl::a_sim_vars(s_sim_top)
  set dir $::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir)
  set flow $::tclapp::aldec::activehdl::a_sim_vars(s_simulation_flow)
  set fs_obj [get_filesets $::tclapp::aldec::activehdl::a_sim_vars(s_simset)]
  ...
  if { [::tclapp::aldec::activehdl::usf_is_axi_bfm_ip] } {
...
proc usf_modelsim_create_do_file_for_simulation { do_file } {
  ...
  set top $::tclapp::aldec::activehdl::a_sim_vars(s_sim_top)
  set dir $::tclapp::aldec::activehdl::a_sim_vars(s_launch_dir)
  set b_batch $::tclapp::aldec::activehdl::a_sim_vars(b_batch)
  set b_scripts_only $::tclapp::aldec::activehdl::a_sim_vars(b_scripts_only)
  set fs_obj [get_filesets $::tclapp::aldec::activehdl::a_sim_vars(s_simset)]
  set fh 0
  if {[catch {open $do_file w} fh]} {
  ...
  # if { {} != $saif } {
    ...
    # if { {} == $uut } {
      # set uut "/$top/uut/*"
    # }
    # if { ({functional} == $::tclapp::aldec::activehdl::a_sim_vars(s_type)) || \
         # ({timing} == $::tclapp::aldec::activehdl::a_sim_vars(s_type)) } {
      # puts $fh "power add -r -in -inout -out -internal [::tclapp::aldec::activehdl::usf_resolve_uut_name uut]\n"
    # } else {
      # puts $fh "power add -in -inout -out -internal [::tclapp::aldec::activehdl::usf_resolve_uut_name uut]\n"
    # }
  # }
...
proc usf_modelsim_write_header { fh filename file_type } {
  ...
  set mode_type   $::tclapp::aldec::activehdl::a_sim_vars(s_mode)
...
proc usf_modelsim_write_driver_shell_script { do_filename step } {
  ...
  set scr_filename $step;append scr_filename [::tclapp::aldec::activehdl::usf_get_script_extn]
...
#    if { {64} == $::tclapp::aldec::activehdl::a_sim_vars(s_int_os_type) } {
#      set s_64bit {-64}
#    }
#  }
  ...
  if {$::tcl_platform(platform) == "unix"} {
    ...
    ::tclapp::aldec::activehdl::usf_write_shell_step_fn $fh_scr
    puts $fh_scr "ExecStep \"$::tclapp::aldec::activehdl::a_sim_vars(s_tool_bin_path)/vsim\" $batch_sw -do \"$do_filename\" -l \"$log_filename\""
  } else {
    puts $fh_scr "@echo off"
    puts $fh_scr "call \"$::tclapp::aldec::activehdl::a_sim_vars(s_tool_bin_path)\\vsim\" $batch_sw -do \"$do_filename\" -l \"$log_filename\""
