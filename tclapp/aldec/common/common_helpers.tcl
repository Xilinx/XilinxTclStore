...
proc usf_set_simulation_flow {} {
  ...
  if { {behavioral} == $a_sim_vars(s_mode) } {
    if { ({functional} == $a_sim_vars(s_type)) || ({timing} == $a_sim_vars(s_type)) } {
      send_msg_id USF-[usf_getSimulatorName]-1 ERROR "Invalid simulation type '$a_sim_vars(s_type)' specified. Please see 'simulate -help' for more details.\n"
      return 1
    }
    ...
    if { ({functional} != $a_sim_vars(s_type)) && ({timing} != $a_sim_vars(s_type)) } {
      send_msg_id USF-[usf_getSimulatorName]-2 ERROR "Invalid simulation type '$a_sim_vars(s_type)' specified. Please see 'simulate -help' for more details.\n"
      return 1
    }
    ...
    send_msg_id USF-[usf_getSimulatorName]-4 ERROR "Invalid simulation mode '$a_sim_vars(s_mode)' specified. Please see 'simulate -help' for more details.\n"
    return 1
  }
...
proc usf_set_sim_tcl_obj {} {
  ...
  send_msg_id USF-[usf_getSimulatorName]-5 INFO "Simulation object is '$a_sim_vars(sp_tcl_obj)'...\n"
  return 0
}
...
proc usf_write_design_netlist {} {
  ...
  switch -regexp -- $a_sim_vars(s_simulation_flow) {
    {post_synth_sim} {
      if { {RTL} == $design_mode } {
        if { [get_param "project.checkRunResultsForUnifiedSim"] } {
          ...
          if { ([regexp -nocase {^synth_design complete} $status] != 1) } {
            send_msg_id USF-[usf_getSimulatorName]-6 ERROR \
      ...
      if { {RTL} == $design_mode } {
        ...
        if { {} != $synth_design } {
          ...
          if { [catch {open_run $synth_run -name $netlist} open_error] } {
            #send_msg_id USF-[usf_getSimulatorName]-7 WARNING "open_run failed:$open_err"
          } else {
        ...
        send_msg_id USF-[usf_getSimulatorName]-8 ERROR "Unsupported design mode found while opening the design for netlist generation!\n"
        return 1
      }

      set design_in_memory [current_design]
      send_msg_id USF-[usf_getSimulatorName]-9 INFO "Writing simulation netlist file for design '$design_in_memory'..."
      # write netlist/sdf
      ...
      if { {.v} == $extn } {
        send_msg_id USF-[usf_getSimulatorName]-10 INFO "write_verilog $wv_args"
        eval "write_verilog $wv_args"
      } else {
        send_msg_id USF-[usf_getSimulatorName]-11 INFO "write_vhdl $wv_args"
        eval "write_vhdl $wv_args"
      }
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id USF-[usf_getSimulatorName]-12 INFO "Writing SDF file..."
        set ws_args "-mode timesim $sdf_cmd_args -file $sdf_file"
        send_msg_id USF-[usf_getSimulatorName]-13 INFO "write_sdf $ws_args"
    ...
    {post_impl_sim} {
      ...
      if { [get_param "project.checkRunResultsForUnifiedSim"] } {
        ...
        if { ![get_property can_open_results $impl_run] } {
          send_msg_id USF-[usf_getSimulatorName]-14 ERROR \
      ...
      if { {} != $impl_design } {
        ...
        if { [catch {open_run $impl_run -name $netlist} open_err] } {
          #send_msg_id USF-[usf_getSimulatorName]-15 WARNING "open_run failed:$open_err"
        } else {
      ...
      set design_in_memory [current_design]
      send_msg_id USF-[usf_getSimulatorName]-16 INFO "Writing simulation netlist file for design '$design_in_memory'..."
      ...
      if { {.v} == $extn } {
        send_msg_id USF-[usf_getSimulatorName]-17 INFO "write_verilog $wv_args"
        eval "write_verilog $wv_args"
      } else {
        send_msg_id USF-[usf_getSimulatorName]-18 INFO "write_vhdl $wv_args"
        eval "write_vhdl $wv_args"
      }
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id USF-[usf_getSimulatorName]-19 INFO "Writing SDF file..."
        set ws_args "-mode timesim $sdf_cmd_args -file $sdf_file"
        send_msg_id USF-[usf_getSimulatorName]-20 INFO "write_sdf $ws_args"
        eval "write_sdf $ws_args"
      }
  ...
  if { [file exist $net_file] } { send_msg_id USF-[usf_getSimulatorName]-21 INFO "Netlist generated:$net_file" }
  if { [file exist $sdf_file] } { send_msg_id USF-[usf_getSimulatorName]-22 INFO "SDF generated:$sdf_file" }
}
...
proc usf_get_compile_order_for_obj { } {
  ...
  if { [usf_is_ip $tcl_obj] } {
    send_msg_id USF-[usf_getSimulatorName]-23 INFO "Inspecting IP design source files for '$a_sim_vars(s_sim_top)'...\n"
    ...
    send_msg_id USF-[usf_getSimulatorName]-24 INFO "Inspecting design source files for '$a_sim_vars(s_sim_top)' in fileset '$tcl_obj'...\n"
    ...
    send_msg_id USF-[usf_getSimulatorName]-25 INFO "Unsupported object source: $tcl_obj\n"
    return 1
  }
}
...
proc usf_copy_glbl_file {} {
  ...
  if {[catch {file copy -force $src_glbl_file $run_dir} error_msg] } {
    send_msg_id USF-[usf_getSimulatorName]-26 WARNING "Failed to copy glbl file '$src_glbl_file' to '$run_dir' : $error_msg\n"
  }
}
...
proc usf_create_do_file { simulator do_filename } {
  ...
  if {[catch {open $do_file w} fh_do]} {
    send_msg_id USF-[usf_getSimulatorName]-27 ERROR "Failed to open file to write ($do_file)\n"
  } else {
...
proc usf_prepare_ip_for_simulation { } {
  ...
  if { [usf_is_fileset $target_obj] } {
    ...
    foreach fs_obj $fs_objs {
      ...
      send_msg_id USF-[usf_getSimulatorName]-28 INFO "Inspecting fileset '$fs_name' for IP generation...\n"
    ...
    if { [usf_is_embedded_flow] } {
      send_msg_id USF-[usf_getSimulatorName]-29 INFO "Design contains embedded sources, generating MEM files for simulation...\n"
    ...
    send_msg_id USF-[usf_getSimulatorName]-30 ERROR "Unknown target '$target_obj'!\n"
  }
  # generate functional netlist  
  if { [llength $runs_to_launch] > 0 } {
    send_msg_id USF-[usf_getSimulatorName]-31 INFO "Launching block-fileset run '$runs_to_launch'...\n"
...
proc usf_generate_mem_files_for_simulation { } {
  ...
  if { [usf_is_fileset $a_sim_vars(sp_tcl_obj)] } {
    # fileset contains embedded sources? generate mem files
    if { [usf_is_embedded_flow] } {
      send_msg_id USF-[usf_getSimulatorName]-32 INFO "Design contains embedded sources, generating MEM files for simulation...\n"
      generate_mem_files $a_sim_vars(s_launch_dir)
    }
  }
}
  ...
  if { {} == $install_path } {
    ...
    if { {} == $bin_path } {
      if { $a_sim_vars(b_scripts_only) } {
        ...
        send_msg_id USF-[usf_getSimulatorName]-34 WARNING \
        ...
        send_msg_id USF-[usf_getSimulatorName]-35 ERROR \
      ...
      send_msg_id USF-[usf_getSimulatorName]-36 INFO "Using simulator executables from '$bin_path'\n"
    }
  } else {
    ...
    if { [file exists $tool_path] && ![file isdirectory $tool_path] } {
      send_msg_id USF-[usf_getSimulatorName]-37 INFO "Using simulator executables from '$tool_path'\n"
    } else {
      send_msg_id USF-[usf_getSimulatorName]-38 ERROR "Path to custom '$tool_name' executable program does not exist:$tool_path'\n"
    }
  }
...
proc usf_get_files_for_compilation_behav_sim { global_files_str_arg } {
  ...
  set incl_files      [list]
  send_msg_id USF-[usf_getSimulatorName]-39 INFO "Finding global include files..."
  ...
  send_msg_id USF-[usf_getSimulatorName]-40 INFO "Finding include directories and verilog header directory paths..."
  ...
  if { [usf_is_fileset $target_obj] } {
    ...
    if { {All} == $src_mgmt_mode } {
      send_msg_id USF-[usf_getSimulatorName]-41 INFO "Fetching design files from '$target_obj'..."
      ...
      if { {} != $linked_src_set } {
        ...
        if { {} != $srcset_obj } {
          ...
          send_msg_id USF-[usf_getSimulatorName]-42 INFO "Fetching design files from '$srcset_obj'...(this may take a while)..."
    ...
    if { $b_add_sim_files } {
      ...
      send_msg_id USF-[usf_getSimulatorName]-43 INFO "Fetching design files from '$a_sim_vars(s_simset)'..."
    ...
    send_msg_id USF-[usf_getSimulatorName]-44 INFO "Fetching design files from IP '$target_obj'..."
    foreach file $::tclapp::aldec::common_helpers::l_compile_order_files {
      set file_type [get_property "FILE_TYPE" [lindex [get_files -quiet -all [list "$file"]] 0]]
      if { ({Verilog} != $file_type) && ({SystemVerilog} != $file_type) && ({VHDL} != $file_type) && ({VHDL 2008} != $file_type) } { continue }
      set g_files $global_files_str
...
proc usf_launch_script { step } {
  ...
  if { $a_sim_vars(b_scripts_only) } {
    send_msg_id USF-[usf_getSimulatorName]-45 INFO "Script generated:[file normalize [file join $run_dir $scr_file]]"
    return 0
  }
  ...
  send_msg_id USF-[usf_getSimulatorName]-46 INFO "Executing '[string toupper $step]' step in '$run_dir'"
  set results_log {}
  switch $step {
    {compile} -
    {elaborate} {
    ...
    {simulate} {
      ...
      if { $retval } {
        send_msg_id USF-[usf_getSimulatorName]-47 ERROR "Failed to launch $scr_file:$error_log\n"
        set faulty_run 1
      }
    }
  }
  ...
  if { $faulty_run } {
    [catch {send_msg_id USF-[usf_getSimulatorName]-48 ERROR "'$step' step failed with error(s). Please check the Tcl console output or '$results_log' file for more information.\n"}]
    # IMPORTANT - *** DONOT MODIFY THIS ***
    error "_SIM_STEP_RUN_EXEC_ERROR_"
    # IMPORTANT - *** DONOT MODIFY THIS ***
    return 1
