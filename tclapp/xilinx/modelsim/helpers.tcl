...
proc usf_write_design_netlist {} {
  ...
  switch -regexp -- $a_sim_vars(s_simulation_flow) {
    {post_synth_sim} {
      ...
      send_msg_id USF-ModelSim-029 INFO "Writing simulation netlist file for design '$design_in_memory'..."
      # write netlist/sdf
    ...
    {post_impl_sim} {
      ...
      set design_in_memory [current_design]
      send_msg_id USF-ModelSim-032 INFO "Writing simulation netlist file for design '$design_in_memory'..."
...
proc usf_get_compile_order_files { } {
  ...
  # Argument Usage:
  # Return Value:
  return [xcs_uniquify_cmd_str $::tclapp::xilinx::modelsim::l_compile_order_files]
}
...
proc usf_compile_glbl_file { simulator b_load_glbl design_files } {
  ...
  set flow        $a_sim_vars(s_simulation_flow)
  if { [xcs_contains_verilog $design_files $flow $a_sim_vars(s_netlist_file)] } {
    if { $b_load_glbl } {
      return 1
    }
    return 0
...
proc usf_get_simulator_lib_for_bfm {} {
  ...
  if { {} != $xil } {
    append platform ".o"
    set lib_path {}
    send_msg_id USF-ModelSim-116 INFO "Finding simulator library from 'XILINX_VIVADO'..."
    foreach path [split $xil $path_sep] {
      set file [file normalize [file join $path "lib" $platform $lib_name]]
      if { [file exists $file] } {
        send_msg_id USF-ModelSim-117 INFO "Using library:'$file'"
        set simulator_lib $file
        break
      } else {
        send_msg_id USF-ModelSim-118 WARNING "Library not found:'$file'"
      }
    }
  } else {
...
proc usf_get_files_from_block_filesets { filter_type } {
  ...
  if { [llength $fs_objs] > 0 } {
    send_msg_id USF-ModelSim-113 INFO "Finding block fileset files..."
    foreach fs_obj $fs_objs {
