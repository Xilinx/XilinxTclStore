...
proc usf_write_design_netlist {} {
  ...
  switch -regexp -- $a_sim_vars(s_simulation_flow) {
    {post_synth_sim} {
      ...
      if { ({Not started} == $status) } {
        send_msg_id Vivado-IES-025 ERROR \
      ...
      send_msg_id Vivado-IES-026 INFO "Generarting simulation netlist '$net_file'"
      # write netlist/sdf
      ...
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id Vivado-IES-027 INFO "Writing SDF file '$sdf_file'"
    ...
    {post_impl_sim} {
      ...
      send_msg_id Vivado-IES-029 INFO "Generarting simulation netlist '$net_file'"
      ...
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id Vivado-IES-030 INFO "Writing SDF file '$sdf_file'"
        write_sdf -mode timesim -force $sdf_file
      }
  ...
  if { [file exist $net_file] } { send_msg_id Vivado-IES-031 INFO "Netlist generated:'$net_file'" }
  if { [file exist $sdf_file] } { send_msg_id Vivado-IES-032 INFO "SDF generated:'$sdf_file'" }
}
...
proc usf_launch_script { simulator step } {
  ...
  if { $a_sim_vars(b_scripts_only) } {
    send_msg_id Vivado-IES-062 INFO "Scripts generated."
    return 0
  }
...
proc usf_generate_comp_file_for_simulation { comp_file runs_to_launch_arg } {
  ...
  send_msg_id Vivado-IES-074 INFO "Processing IP '$ip_name' for simulation models/functional netlist...\n"
  ...
  if { [get_property "IS_IP_BEHAV_LANG_SUPPORTED" $comp_file] } {
    send_msg_id Vivado-IES-075 INFO "IP supports '$ts' language simulation models\n"
    # does ip generated simulation products? if not, generate them
    if { ![get_property "IS_IP_GENERATED_SIM" $comp_file] } {
      send_msg_id Vivado-IES-076 INFO "IP is not generated for simulation. Generating simulation products...\n"
      generate_target {simulation} [get_files $comp_file] -force
    } else {
      send_msg_id Vivado-IES-077 INFO "IP is upto date for simulation\n"
    }
  } elseif { [get_property "GENERATE_SYNTH_CHECKPOINT" $comp_file] } {
    send_msg_id Vivado-IES-078 INFO "IP 'GENERATE_SYNTH_CHECKPOINT' is set\n"
    # make sure ip is up-to-date
    if { ![get_property "IS_IP_GENERATED" $comp_file] } { 
      send_msg_id Vivado-IES-079 INFO "Generating 'all' products...\n"
      generate_target {all} [get_files $comp_file] -force
      send_msg_id Vivado-IES-080 INFO "Generating 'netlist' with IP block-fileset runs...\n"
      usf_generate_ip_netlist $comp_file runs_to_launch
    } else {
      send_msg_id Vivado-IES-081 INFO "IP is upto date for all products\n"
    }
  } else {
    ...
    send_msg_id Vivado-IES-082 WARNING "$error_msg\n"
    #return 1
  }
  send_msg_id Vivado-IES-083 INFO "Processing complete:'$ip_name'\n"
}
