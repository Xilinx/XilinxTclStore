...
proc usf_set_simulation_flow {} {
  ...
  if { {behavioral} == $a_sim_vars(s_mode) } {
    if { ({functional} == $a_sim_vars(s_type)) || ({timing} == $a_sim_vars(s_type)) } {
      send_msg_id USF-[getSimulatorName]-1 ERROR "Invalid simulation type '$a_sim_vars(s_type)' specified. Please see 'simulate -help' for more details.\n"
      return 1
    }
    ...
    if { ({functional} != $a_sim_vars(s_type)) && ({timing} != $a_sim_vars(s_type)) } {
      send_msg_id USF-[getSimulatorName]-2 ERROR "Invalid simulation type '$a_sim_vars(s_type)' specified. Please see 'simulate -help' for more details.\n"
      return 1
    }
    ...
    if { ({functional} != $a_sim_vars(s_type)) && ({timing} != $a_sim_vars(s_type)) } {
      send_msg_id USF-[getSimulatorName]-3 ERROR "Invalid simulation type '$a_sim_vars(s_type)' specified. Please see 'simulate -help' for more details.\n"
      return 1
    }
    ...
    send_msg_id USF-[getSimulatorName]-4 ERROR "Invalid simulation mode '$a_sim_vars(s_mode)' specified. Please see 'simulate -help' for more details.\n"
    return 1
  }
...
proc usf_set_sim_tcl_obj {} {
  ...
  if { {} != $comp_file } {
    ...
    set a_sim_vars(sp_tcl_obj) [get_filesets $::tclapp::aldec::riviera::a_sim_vars(s_simset)]
    # set current simset
  ...
  send_msg_id USF-[getSimulatorName]-5 INFO "Simulation object is '$a_sim_vars(sp_tcl_obj)'...\n"
  return 0
}
...
proc usf_write_design_netlist {} {
  ...
  switch -regexp -- $a_sim_vars(s_simulation_flow) {
    {post_synth_sim} {
      if { {RTL} == $design_mode } {
        if { [get_param "project.checkRunResultsForUnifiedSim"] } {
          ...
          if { ([regexp -nocase {^synth_design complete} $status] != 1) } {
            send_msg_id USF-[getSimulatorName]-6 ERROR \
      ...
      if { {RTL} == $design_mode } {
        ...
        if { {} != $synth_design } {
          ...
          if { [catch {open_run $synth_run -name $netlist} open_error] } {
            #send_msg_id USF-[getSimulatorName]-7 WARNING "open_run failed:$open_err"
          } else {
        ...
        send_msg_id USF-[getSimulatorName]-8 ERROR "Unsupported design mode found while opening the design for netlist generation!\n"
        return 1
      }

      set design_in_memory [current_design]
      send_msg_id USF-[getSimulatorName]-9 INFO "Writing simulation netlist file for design '$design_in_memory'..."
      # write netlist/sdf
      ...
      if { {.v} == $extn } {
        send_msg_id USF-[getSimulatorName]-10 INFO "write_verilog $wv_args"
        eval "write_verilog $wv_args"
      } else {
        send_msg_id USF-[getSimulatorName]-11 INFO "write_vhdl $wv_args"
        eval "write_vhdl $wv_args"
      }
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id USF-[getSimulatorName]-12 INFO "Writing SDF file..."
        set ws_args "-mode timesim $sdf_cmd_args -file $sdf_file"
        send_msg_id USF-[getSimulatorName]-13 INFO "write_sdf $ws_args"
    ...
    {post_impl_sim} {
      ...
      if { [get_param "project.checkRunResultsForUnifiedSim"] } {
        ...
        if { ![get_property can_open_results $impl_run] } {
          send_msg_id USF-[getSimulatorName]-14 ERROR \
      ...
      if { {} != $impl_design } {
        ...
        if { [catch {open_run $impl_run -name $netlist} open_err] } {
          #send_msg_id USF-[getSimulatorName]-15 WARNING "open_run failed:$open_err"
        } else {
      ...
      set design_in_memory [current_design]
      send_msg_id USF-[getSimulatorName]-16 INFO "Writing simulation netlist file for design '$design_in_memory'..."
      ...
      if { {.v} == $extn } {
        send_msg_id USF-[getSimulatorName]-17 INFO "write_verilog $wv_args"
        eval "write_verilog $wv_args"
      } else {
        send_msg_id USF-[getSimulatorName]-18 INFO "write_vhdl $wv_args"
        eval "write_vhdl $wv_args"
      }
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id USF-[getSimulatorName]-19 INFO "Writing SDF file..."
        set ws_args "-mode timesim $sdf_cmd_args -file $sdf_file"
        send_msg_id USF-[getSimulatorName]-20 INFO "write_sdf $ws_args"
        eval "write_sdf $ws_args"
      }
  ...
  if { [file exist $net_file] } { send_msg_id USF-[getSimulatorName]-21 INFO "Netlist generated:$net_file" }
  if { [file exist $sdf_file] } { send_msg_id USF-[getSimulatorName]-22 INFO "SDF generated:$sdf_file" }
}
...
proc usf_get_compile_order_for_obj { } {
  ...
  if { [usf_is_ip $tcl_obj] } {
    send_msg_id USF-[getSimulatorName]-23 INFO "Inspecting IP design source files for '$a_sim_vars(s_sim_top)'...\n"
    ...
    send_msg_id USF-[getSimulatorName]-24 INFO "Inspecting design source files for '$a_sim_vars(s_sim_top)' in fileset '$tcl_obj'...\n"
    ...
    send_msg_id USF-[getSimulatorName]-25 INFO "Unsupported object source: $tcl_obj\n"
    return 1
  }
}
...
proc usf_copy_glbl_file {} {
  ...
  if {[catch {file copy -force $src_glbl_file $run_dir} error_msg] } {
    send_msg_id USF-[getSimulatorName]-26 WARNING "Failed to copy glbl file '$src_glbl_file' to '$run_dir' : $error_msg\n"
  }
}
...
proc usf_create_do_file { simulator do_filename } {
  ...
  if {[catch {open $do_file w} fh_do]} {
    send_msg_id USF-[getSimulatorName]-27 ERROR "Failed to open file to write ($do_file)\n"
  } else {
...
proc usf_prepare_ip_for_simulation { } {
  ...
  if { [usf_is_fileset $target_obj] } {
    ...
    foreach fs_obj $fs_objs {
      ...
      send_msg_id USF-[getSimulatorName]-28 INFO "Inspecting fileset '$fs_name' for IP generation...\n"
    ...
    if { [usf_is_embedded_flow] } {
      send_msg_id USF-[getSimulatorName]-29 INFO "Design contains embedded sources, generating MEM files for simulation...\n"
    ...
    send_msg_id USF-[getSimulatorName]-30 ERROR "Unknown target '$target_obj'!\n"
  }
  # generate functional netlist  
  if { [llength $runs_to_launch] > 0 } {
    send_msg_id USF-[getSimulatorName]-31 INFO "Launching block-fileset run '$runs_to_launch'...\n"
...
proc usf_generate_mem_files_for_simulation { } {
  ...
  if { [usf_is_fileset $a_sim_vars(sp_tcl_obj)] } {
    # fileset contains embedded sources? generate mem files
    if { [usf_is_embedded_flow] } {
      send_msg_id USF-[getSimulatorName]-32 INFO "Design contains embedded sources, generating MEM files for simulation...\n"
      generate_mem_files $a_sim_vars(s_launch_dir)
    }
  }
}
...
proc usf_set_simulator_path { simulator } {
  ...
  send_msg_id USF-[getSimulatorName]-33 INFO "Finding simulator installation...\n"
  ...
  if { {} == $install_path } {
    ...
    if { {} == $bin_path } {
      if { $a_sim_vars(b_scripts_only) } {
        ...
        send_msg_id USF-[getSimulatorName]-34 WARNING \
        ...
        send_msg_id USF-[getSimulatorName]-35 ERROR \
      ...
      send_msg_id USF-[getSimulatorName]-36 INFO "Using simulator executables from '$bin_path'\n"
    }
  } else {
    ...
    if { [file exists $tool_path] && ![file isdirectory $tool_path] } {
      send_msg_id USF-[getSimulatorName]-37 INFO "Using simulator executables from '$tool_path'\n"
    } else {
      send_msg_id USF-[getSimulatorName]-38 ERROR "Path to custom '$tool_name' executable program does not exist:$tool_path'\n"
    }
  }
...
proc usf_get_files_for_compilation_behav_sim { global_files_str_arg } {
  ...
  set files          [list]
  set target_obj     $a_sim_vars(sp_tcl_obj)
  set simset_obj     [get_filesets $::tclapp::aldec::riviera::a_sim_vars(s_simset)]
  ...
  set incl_files      [list]
  send_msg_id USF-[getSimulatorName]-39 INFO "Finding global include files..."
  ...
  send_msg_id USF-[getSimulatorName]-40 INFO "Finding include directories and verilog header directory paths..."
  ...
  if { [usf_is_fileset $target_obj] } {
    ...
    if { {All} == $src_mgmt_mode } {
      send_msg_id USF-[getSimulatorName]-41 INFO "Fetching design files from '$target_obj'..."
      ...
      if { {} != $linked_src_set } {
        ...
        if { {} != $srcset_obj } {
          ...
          send_msg_id USF-[getSimulatorName]-42 INFO "Fetching design files from '$srcset_obj'...(this may take a while)..."
    ...
    if { $b_add_sim_files } {
      ...
      send_msg_id USF-[getSimulatorName]-43 INFO "Fetching design files from '$a_sim_vars(s_simset)'..."
    ...
    send_msg_id USF-[getSimulatorName]-44 INFO "Fetching design files from IP '$target_obj'..."
    foreach file $::tclapp::aldec::riviera::l_compile_order_files {
      set file_type [get_property "FILE_TYPE" [lindex [get_files -quiet -all [list "$file"]] 0]]
      if { ({Verilog} != $file_type) && ({SystemVerilog} != $file_type) && ({VHDL} != $file_type) && ({VHDL 2008} != $file_type) } { continue }
      set g_files $global_files_str
...
proc usf_launch_script { simulator step } {
  ...
  if { $a_sim_vars(b_scripts_only) } {
    send_msg_id USF-[getSimulatorName]-45 INFO "Script generated:[file normalize [file join $run_dir $scr_file]]"
    return 0
  }
  ...
  send_msg_id USF-[getSimulatorName]-46 INFO "Executing '[string toupper $step]' step in '$run_dir'"
  set results_log {}
  switch $step {
    {compile} -
    {elaborate} {
    ...
    {simulate} {
      ...
      if { $retval } {
        send_msg_id USF-[getSimulatorName]-47 ERROR "Failed to launch $scr_file:$error_log\n"
        set faulty_run 1
      }
    }
  }
  ...
  if { $faulty_run } {
    [catch {send_msg_id USF-[getSimulatorName]-48 ERROR "'$step' step failed with error(s). Please check the Tcl console output or '$results_log' file for more information.\n"}]
    # IMPORTANT - *** DONOT MODIFY THIS ***
    error "_SIM_STEP_RUN_EXEC_ERROR_"
    # IMPORTANT - *** DONOT MODIFY THIS ***
    return 1
...
proc usf_get_simulator_lib_for_bfm {} {
  ...
  set path_sep      {;}
  set lib_extn      {.dll}
  set platform      [::tclapp::aldec::riviera::usf_get_platform]
  ...
  if { {} != $xil } {
    ...
    send_msg_id USF-[getSimulatorName]-49 ERROR "Environment variable 'XILINX_VIVADO' is not set!"
  }
...
proc usf_get_netlist_extn { warning } {
  ...
  if { (({VHDL} == $target_lang) && ({timing} == $a_sim_vars(s_type))) } {
    ...
    if { $warning } {
      ...
      send_msg_id USF-[getSimulatorName]-50 INFO "The target language is set to VHDL, it is not supported for simulation type '$a_sim_vars(s_type)', using Verilog instead.\n"
    }
  }
  return $extn
}
  ...
  if { [llength $data_files] > 0 } {
    ...
    foreach file $data_files {
      if {[catch {file copy -force $file $export_dir} error_msg] } {
        send_msg_id USF-[getSimulatorName]-51 WARNING "Failed to copy file '$file' to '$export_dir' : $error_msg\n"
      } else {
        send_msg_id USF-[getSimulatorName]-52 INFO "Exported '$file'\n"
      }
    }
  }
}
...
proc usf_get_files_from_block_filesets { filter_type } {
  ...
  if { [llength $fs_objs] > 0 } {
    send_msg_id USF-[getSimulatorName]-53 INFO "Finding block fileset files..."
    foreach fs_obj $fs_objs {
      set fs_name [get_property "NAME" $fs_obj]
      send_msg_id USF-[getSimulatorName]-54 INFO "Inspecting fileset '$fs_name' for '$filter_type' files...\n"
      ...
      if { [llength $files] == 0 } {
        send_msg_id USF-[getSimulatorName]-55 INFO "No files found in '$fs_name'\n"
        continue
      } else {
        foreach file $files {
...
proc usf_make_file_executable { file } {
  ...
  if {$::tcl_platform(platform) == "unix"} {
    if {[catch {exec chmod a+x $file} error_msg] } {
      send_msg_id USF-[getSimulatorName]-56 WARNING "Failed to change file permissions to executable ($file): $error_msg\n"
    }
  } else {
    if {[catch {exec attrib /D -R $file} error_msg] } {
      send_msg_id USF-[getSimulatorName]-57 WARNING "Failed to change file permissions to executable ($file): $error_msg\n"
    }
  }
}
...
proc usf_generate_comp_file_for_simulation { comp_file runs_to_launch_arg } {
  ...
  if { [get_property "IS_IP_BEHAV_LANG_SUPPORTED" $comp_file] } {
    ...
    if { ![get_property "IS_IP_GENERATED_SIM" $comp_file] } {
      send_msg_id USF-[getSimulatorName]-58 INFO "Generating simulation products for IP '$ip_name'...\n"
      ...
      send_msg_id USF-[getSimulatorName]-59 INFO "IP '$ip_name' is upto date for simulation\n"
    }
    ...
    if { ![get_property "IS_IP_GENERATED" $comp_file] } {
      ...
      send_msg_id USF-[getSimulatorName]-60 INFO "Generating functional netlist for IP '$ip_name'...\n"
      usf_generate_ip_netlist $comp_file runs_to_launch
    } else {
      send_msg_id USF-[getSimulatorName]-61 INFO "IP '$ip_name' is upto date for all products\n"
    }
  } else {
    ...
    send_msg_id USF-[getSimulatorName]-62 WARNING "$error_msg\n"
    #return 1
  }
}
...
proc usf_generate_ip_netlist { comp_file runs_to_launch_arg } {
  ...
  if { ![get_property "GENERATE_SYNTH_CHECKPOINT" $comp_file_obj] } {
    send_msg_id USF-[getSimulatorName]-63 INFO "Generate synth checkpoint is 'false':$comp_file\n"
    # if synth checkpoint read-only, return
    if { [get_property "IS_IP_SYNTH_CHECKPOINT_READONLY" $comp_file_obj] } {
      send_msg_id USF-[getSimulatorName]-64 WARNING "Synth checkpoint property is 'readonly' ... skipping:$comp_file\n"
      return
    }
    # set property to create a DCP/structural simulation file
    send_msg_id USF-[getSimulatorName]-65 INFO "Setting synth checkpoint for generating simulation netlist:$comp_file\n"
    set_property "GENERATE_SYNTH_CHECKPOINT" true $comp_file_obj
  } else {
    send_msg_id USF-[getSimulatorName]-66 INFO "Generate synth checkpoint is set:$comp_file\n"
  }
  ...
  if { {} == $block_fs_obj } {
    ...
    send_msg_id USF-[getSimulatorName]-67 INFO "Block-fileset created:$block_fs_obj"
    # set fileset top
    ...
    send_msg_id USF-[getSimulatorName]-68 INFO "Moving ip composite source(s) to '$ip_basename' fileset"
  ...
  if { {BlockSrcs} != [get_property "FILESET_TYPE" $block_fs_obj] } {
    send_msg_id USF-[getSimulatorName]-69 ERROR "Given source file is not associated with a design source fileset.\n"
    return 1
  }
  ...
  send_msg_id USF-[getSimulatorName]-70 INFO "Run scheduled for '$ip_basename':$run_name\n"
}
...
proc usf_check_errors { step results_log_arg } {
  ...
  if {[catch {open $log r} fh]} {
    send_msg_id USF-[getSimulatorName]-71 WARNING "Failed to open file to read ($log)\n"
    close $fh
  } else {
  ...
  if { $retval } {
    [catch {send_msg_id USF-[getSimulatorName]-72 INFO "Step results log file:'$log'\n"}]
    return 1
  }
  return 0
}
...
proc usf_get_top { top_arg } {
  ...
  if { {} == $top } {
    send_msg_id USF-[getSimulatorName]-73 ERROR "Top module not set for fileset '$fs_name'. Please ensure that a valid \
