...
proc usf_set_simulator_path { simulator } {
 ...
 if { {} == $install_path } {
   ...
   send_msg_id Vivado-IES-999 INFO "Using simulator executables from '$bin_path'\n"
 } else {
...
proc usf_get_netlist_extn { warning } {
  ...
  if { ({VHDL} == $target_lang) && ({timing} == $a_sim_vars(s_type) || {functional} == $a_sim_vars(s_type)) } {
    set extn {.v}
    if { $warning } {
      send_msg_id Vivado-IES-999 INFO "The target language is set to VHDL, it is not supported for simulation type '$a_sim_vars(s_type)', using Verilog instead.\n"
    }
  }
  return $extn
}
