# Usage: write_questa_rdc_script <top_module> [-output_directory <output_directory>] [-use_existing_xdc|-no_sdc]
###############################################################################
#
...
namespace eval ::tclapp::mentor::questa_rdc {
  # Export procs that should be allowed to import into other namespaces
  variable QUESTA_RDC_TCL_SCRIPT_PATH [file normalize [file dirname [info script]]]
  namespace export write_questa_rdc_script
}

proc ::tclapp::mentor::questa_rdc::matches_default_libs {lib} {
  ...
  # Categories: xilinxtclstore, mentor, questa_rdc
...
proc ::tclapp::mentor::questa_rdc::uniquify_lib {lib lang num} {
...
proc ::tclapp::mentor::questa_rdc::write_questa_rdc_script {args} {
  ...
  set add_button 0
  set remove_button 0
  set usage_msg "Usage    : write_questa_rdc_script <top_module> \[-output_directory <out_dir>\] \[-use_existing_xdc|-no_sdc\] \[-run <report_clock|rdc_run>\] \[-add_button\] \[-remove_button\]"
  # Parse the arguments
  if { [llength $args] > 8 } {
  ...
  for {set i 0} {$i < [llength $args]} {incr i} {
    if { [lindex $args $i] == "-output_directory" } {
      ...
      if { ($run_questa_rdc != "rdc_run") && ($run_questa_rdc != "report_clock") } {
  ...
  if { $add_button == 1 } {
    ...
    ## -----------------------------------------
    ## 0=Run%20Questa%20RDC tclapp::mentor::questa_rdc::write_questa_rdc_script "" /home/iahmed/questa_rdc_logo.PNG "" "" true ^@ "" true 4 Top%20Module "" "" false Output%20Directory "" -output_directory%20OD1 true Use%20Existing%20XDC "" -use_existing_xdc true Invoke%20Questa%20RDC%20Run "" -run true
    ## -----------------------------------------
    ...
    variable QUESTA_RDC_TCL_SCRIPT_PAT
    set questa_rdc_logo "$QUESTA_RDC_TCL_SCRIPT_PATH/questa_rdc_logo.PNG"
    ...
    if { [catch {open $commands_file a} result] } {
      puts stderr "ERROR: Could not open commands.paini to add the Questa RDC button, path '$commands_file'\n$result"
      set rc 9
      return $rc
    } else {
    ...
    if { $vivado_cmds_version == 1 } {
      set button_code "$questa_rdc_command_index=Run%20Questa%20RDC"
			 ...
			 set button_code "$button_code source%20\$::env(QHOME)/share/fpga_libs/Xilinx/write_questa_rdc_script.tcl;%20tclapp::mentor::questa_rdc::write_questa_rdc_script"
...
#      set button_code "$button_code source%20\$::env(QHOME)/share/fpga_libs/Xilinx/write_questa_rdc_script.tcl;%20tclapp::mentor::questa_rdc::write_questa_rdc_script"
  ...
  if { $remove_button == 1 } {
    ...
    foreach ip_line $ip_lines {
      ...
      if { [regexp {Questa.*RDC.*write_questa_rdc_script.tcl} $ip_line] } {
...
#            }
  ...
  puts $qrdc_ctrl_fh "resetcheck preference -print_port_domain_template"
  puts $qrdc_ctrl_fh "resetcheck preference tree -sync_internal "
...
## Dump the run Makefile
  ...
  puts $qrdc_run_makefile_fh "\tresetcheck run -d \$(DUT) $lib_args; \\"
  ...
  puts $qrdc_run_batfile_fh "\tqverify -c -licq -l qrdc_${top_module}.log -od $rdc_out_dir -do ^\"do $qrdc_ctrl;do $qrdc_compile_tcl;do $run_sdcfile^\""
  ...
  puts $qrdc_run_batfile_fh "\tqverify -c -licq -l qrdc_${top_module}.log -od $rdc_out_dir -do ^\"do $qrdc_ctrl;resetcheck run -d %DUT% $lib_args; ^\""
  puts $qrdc_run_batfile_fh "\texit /b"
  puts $qrdc_run_batfile_fh ""
  ...
  if { $run_questa_rdc == "report_clock" } { 
    puts $qrdc_tcl_fh "resetcheck run -d $top_module $lib_args -report_clock"
  } else {
    puts $qrdc_tcl_fh "resetcheck run -d $top_module $lib_args"
    puts $qrdc_tcl_fh "resetcheck generate report ${top_module}_detailed.rpt"
  }
...
#  puts $qrdc_tcl_fh "do $qrdc_ctrl"
  ...
  if { $run_questa_rdc == "rdc_run" } {
    ...
    exec qverify -l qverify_ui.log $userOD/RDC_RESULTS/resetcheck.db &
  } elseif { $run_questa_rdc == "report_clock" } {
    puts "INFO : Running Questa RDC (Command: rdc run -report_clock), the UI will be invoked when the run is finished"
    puts "     : Log can be found at $userOD/RDC_RESULTS/qverify.log"
    exec /bin/sh -c "cd $userOD; sh qrdc_run.sh"
    puts "INFO : Questa RDC run is finished"
    puts "INFO : Invoking Questa RDC UI for debugging."
...
namespace import tclapp::mentor::questa_rdc::*
