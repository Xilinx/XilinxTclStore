...
proc usf_xcelium_setup_simulation { args } {
  ...
  set clibs_dir [usf_xcelium_verify_compiled_lib]
  ...
  for {set i 0} {$i < [llength $args]} {incr i} {
    ...
    switch -regexp -- $option {
      ...
      default {
        ...
        if { [regexp {^-} $option] } {
          send_msg_id USF-Xcelium-005 ERROR "Unknown option '$option', please type 'launch_simulation -help' for usage info.\n"
          return 1
        }
      }
    }
...
proc usf_xcelium_set_initial_cmd { fh_scr cmd_str src_file file_type lib prev_file_type_arg prev_lib_arg } {
...
proc usf_xcelium_write_compile_script {} {
  ...
  foreach file $::tclapp::xilinx::xcelium::a_sim_vars(l_design_files) {
    ...
    if { $b_group_files } {
      if { $b_first } {
        set b_first false
        usf_xcelium_set_initial_cmd $fh_scr $cmd_str $src_file $file_type $lib prev_file_type prev_lib
      } else {
  ...
  if { {behav_sim} == $::tclapp::xilinx::xcelium::a_sim_vars(s_simulation_flow) } {
    ...
    if { [xcs_compile_glbl_file "xcelium" $b_load_glbl $a_sim_vars(l_design_files) $a_sim_vars(s_simset) $a_sim_vars(s_simulation_flow) $a_sim_vars(s_netlist_file)] } {
