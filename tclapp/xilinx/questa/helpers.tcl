...
proc usf_get_files_for_compilation_post_sim { global_files_str_arg } {
  ...
  if { $a_sim_vars(b_enable_netlist_sim) && $a_sim_vars(b_netlist_sim) && ({functional} == $a_sim_vars(s_type)) } {
    usf_add_netlist_sources files l_compile_order_files l_incl_dirs_opts l_dummy_incl_dirs_opts
  }

  if { {} != $a_sim_vars(s_netlist_file) } {
    ...
    set cmd_str [usf_get_file_cmd_str $a_sim_vars(s_netlist_file) $file_type false {} l_incl_dirs_opts l_dummy_incl_dirs_opts]
    if { {} != $cmd_str } {
      lappend files $cmd_str
      lappend l_compile_order_files $a_sim_vars(s_netlist_file)
    }
  }
  ...
  if { $a_sim_vars(b_enable_netlist_sim) && $a_sim_vars(b_netlist_sim) && ({functional} == $a_sim_vars(s_type)) } {
    foreach file_obj [get_files -compile_order sources -used_in simulation -of_objects [get_filesets $a_sim_vars(s_simset)]] {
      if { [get_property -quiet "netlist_simulation" $file_obj] } {
        ...
        set cmd_str [usf_get_file_cmd_str $file_obj $file_type false {} l_incl_dirs_opts l_dummy_incl_dirs_opts]
        if { {} != $cmd_str } {
          lappend files $cmd_str
...
proc usf_launch_script { simulator step } {
  ...
  if { $a_sim_vars(b_scripts_only) } {
    send_msg_id USF-Questa-068 INFO "Script generated:[file normalize [file join $a_sim_vars(s_launch_dir) $scr_file]]"
    return 0
  }
  ...
  send_msg_id USF-Questa-069 INFO "Executing '${display_step}' step in '$a_sim_vars(s_launch_dir)'"
  set results_log {}
  switch $step {
    {compile} -
    {elaborate} {
...
proc usf_append_compiler_options { tool file_type opts_arg } {
  ...
  switch $tool {
    "vcom" {
      set vhdl_syntax [get_property "questa.compile.vhdl_syntax" $a_sim_vars(fs_obj)]
      ...
      if { [get_param "project.writeNativeScriptForUnifiedSimulation"] } {
        ...
        set more_options [string trim [get_property "questa.compile.vcom.more_options" $a_sim_vars(fs_obj)]]
        if { {} != $more_options } {
    ...
    "vlog" {
      if { [get_param "project.writeNativeScriptForUnifiedSimulation"] } {
        ...
        set more_options [string trim [get_property "questa.compile.vlog.more_options" $a_sim_vars(fs_obj)]]
        if { {} != $more_options } {
          set arg_list [linsert $arg_list end "$more_options"]
        }
        set cmd_str [join $arg_list " "]
    ...
    "sccom" {
      if { $a_sim_vars(b_int_systemc_mode) } {
        ...
        set cores [string tolower [get_property "questa.compile.sccom.cores" $a_sim_vars(fs_obj)]]
        if { {off} != $cores } {
...
proc usf_add_netlist_sources { files_arg l_compile_order_files_arg l_incl_dirs_opts_arg l_dummy_incl_dirs_opts_arg } {
  # Summary:
  # Argument Usage:
  # Return Value:
  ...
  upvar $l_incl_dirs_opts_arg l_incl_dirs_opts 
  upvar $l_dummy_incl_dirs_opts_arg l_dummy_incl_dirs_opts
  ...
  foreach nf $l_all_netlist_files {
    ...
    if { ![info exists a_sim_noc_files_info($file_name)] } {
      ...
      set cmd_str [usf_get_file_cmd_str $nf_obj $file_type false {} l_incl_dirs_opts l_dummy_incl_dirs_opts]
      if { {} != $cmd_str } {
        lappend files $cmd_str
  ...
  if { {} != $xlnoc_file_obj } {
    set file_type "Verilog"
    set cmd_str [usf_get_file_cmd_str $xlnoc_file_obj $file_type false {} l_incl_dirs_opts l_dummy_incl_dirs_opts]
    if { {} != $cmd_str } {
      lappend files $cmd_str
