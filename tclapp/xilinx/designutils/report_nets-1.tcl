...
# HEADER_BEGIN
# COPYRIGHT NOTICE
# Copyright 2001-2022 Advanced Micro Devices, Inc. All Rights Reserved.
...
##
## Version:        2022.12.06
## Description:    Generate a report for a list of nets
##
########################################################################################

########################################################################################
## 2022.12.06 - Added support for the environment variable TOOLBOX_PATH to access the toolbox
## 2022.10.31 - Improved support for Versal devices (-plot)
## 2022.07.11 - Remove some dependency from the Tcl package 'prettyTable'
## 2022.06.20 - Fixed rare issue with plotting ancestor
## 2022.04.20 - Fixed the inter-slr column in the summary table
## 2022.03.02 - Fixed rare case when nets are not found
## 2020.05.29 - Fixed minor issue with plot and CLOCK_ROOT
## 2020.05.01 - Added support for -load_pins
##            - Fixed issue with -slack
## 2020.01.07 - Added support for -show_ancestor
## 2020.01.03 - Added the number of unplaced loads inside the title of the plot graph
...
if {[catch {
  # Try to load the toolbox
  if {[catch { lappend auto_path $::env(TOOLBOX_PATH) }]} { lappend auto_path /home/dpefour/git/scripts/toolbox }
  package require prettyTable
} errorstring]} {
  # circular package dependency: attempt to provide toolbox 0.1 requires toolbox
  if {![regexp {circular package dependency} $errorstring]} {
    # If it fails, check if prettyTable.tcl is inside the same directory as report_nets.tcl
    if {[catch { set dir [file dirname [file readlink [info script]]] } errorstring]} {
      set dir [file dirname [info script]]
    }
    foreach file [list prettyTable.tcl] {
      if {[file exists $dir/$file]} {
        puts " -I- sourcing $dir/$file"
        source $dir/$file
      } else {
        puts " -E- ERROR - cannot find $dir/$file"
      }
    }
  }
}
...
namespace eval ::tclapp::xilinx::designutils {
  namespace export -force report_nets
}
...
#   $tbl configure -align_right
  ...
  foreach load [get_cells -quiet -of $loads] {
    ...
    if {[regexp {^(S([0-9]))?X([0-9]+)Y([0-9]+)$} $region - - S X Y]} {
      foreach {X Y} [::ttclapp::xilinx::designutils::prettyTable::deviceview::region2XY $tbl $region] { break }
      $tbl incrcell $X $Y
    } elseif {$region == {}} {
