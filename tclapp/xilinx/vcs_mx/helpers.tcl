...
proc usf_write_design_netlist {} {
  ...
  switch -regexp -- $a_sim_vars(s_simulation_flow) {
    {post_synth_sim} {
      if { {RTL} == $design_mode } {
        ...
        if { ([regexp -nocase {^synth_design complete} $status] != 1) } {
          send_msg_id Vivado-VCS_MX-028 ERROR \
      ...
      if { {RTL} == $design_mode } {
        ...
        send_msg_id Vivado-VCS_MX-028 ERROR "Unsupported design mode found while opening the design for netlist generation!\n"
        return 1
      }

      send_msg_id Vivado-VCS_MX-029 INFO "Writing simulation netlist file..."
      # write netlist/sdf
      ...
      send_msg_id Vivado-VCS_MX-999 INFO "write_verilog $wv_args"
      eval "write_verilog $wv_args"
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id Vivado-VCS_MX-030 INFO "Writing SDF file..."
        set ws_args "-mode timesim $sdf_cmd_args -file $sdf_file"
        send_msg_id Vivado-VCS_MX-999 INFO "write_sdf $ws_args"
    ...
    {post_impl_sim} {
      ...
      if { ([regexp -nocase {^route_design complete} $status] != 1) } {
        send_msg_id Vivado-VCS_MX-031 ERROR \
      ...
      send_msg_id Vivado-VCS_MX-032 INFO "Writing simulation netlist file..."
      ...
      eval "write_verilog $wv_args"
      if { {timing} == $a_sim_vars(s_type) } {
        send_msg_id Vivado-VCS_MX-033 INFO "Writing SDF file..."
        set ws_args "-mode timesim $sdf_cmd_args -file $sdf_file"
        send_msg_id Vivado-VCS_MX-999 INFO "write_sdf $ws_args"
        eval "write_sdf $ws_args"
      }
  ...
  if { [file exist $net_file] } { send_msg_id Vivado-VCS_MX-034 INFO "Netlist generated:$net_file" }
  if { [file exist $sdf_file] } { send_msg_id Vivado-VCS_MX-035 INFO "SDF generated:$sdf_file" }
}
...
proc usf_create_do_file { simulator do_filename } {
  ...
  if {[catch {open $do_file w} fh_do]} {
    ...
    set saif {}
    set uut [get_property "VCS_MX.SIMULATE.UUT" $fs_obj]
...
proc usf_launch_script { simulator step } {
  ...
  if { $a_sim_vars(b_scripts_only) } {
    send_msg_id Vivado-VCS_MX-061 INFO "Script generated:[file normalize [file join $run_dir $scr_file]]"
    return 0
  }
...
proc usf_generate_comp_file_for_simulation { comp_file runs_to_launch_arg } {
  ...
  if { [get_property "IS_IP_BEHAV_LANG_SUPPORTED" $comp_file] } {
    ...
    if { ![get_property "IS_IP_GENERATED_SIM" $comp_file] } {
      send_msg_id Vivado-VCS_MX-071 INFO "Generating simulation products for IP '$ip_name'...\n"
      ...
      send_msg_id Vivado-VCS_MX-074 INFO "IP '$ip_name' is upto date for simulation\n"
    }
    ...
    if { ![get_property "IS_IP_GENERATED" $comp_file] } {
      ...
      send_msg_id Vivado-VCS_MX-077 INFO "Generating functional netlist for IP '$ip_name'...\n"
      usf_generate_ip_netlist $comp_file runs_to_launch
    } else {
      send_msg_id Vivado-VCS_MX-078 INFO "IP '$ip_name' is upto date for all products\n"
    }
  } else {
    ...
    send_msg_id Vivado-VCS_MX-079 WARNING "$error_msg\n"
    #return 1
  }
}
